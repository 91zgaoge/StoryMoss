...
[![Version](https://img.shields.io/badge/version-v0.19.0-gold)](./CHANGELOG.md)
[![License](https://img.shields.io/badge/license-ISC-blue.svg)](./LICENSE)

**最新动态**：v0.19.0 提示词全面可配置化——彻底消灭所有硬编码 LLM 提示词，35+ 内置提示词全部纳入统一注册表，支持 15 个分类管理。前端 PromptsPanel 全面重构：分类折叠、实时搜索、批量重置、默认值预览、模板变量高亮。雪花法 10 步、5 个内置技能、Memory/Knowledge/MultiAgent 全部接入注册表。`cargo test --lib` 392/392 通过。

> **上一版**：v0.18.1 设置超时修复——修复后台设置数字输入过快保存问题（改为 onBlur 保存），修复后端 3 处配置读取路径错误（使用 app_data_dir 而非 current_dir），用户设置的超时时间现在真正生效。v0.18.0 后台资产深度审计 × 智能创作流程全面优化——发现并修复系统级矛盾（默认续写路径 TimeSliced 绕过约 90% 后台资产）。4 项断环修复，4 项核心优化，3 项死代码清理，3 项架构优化。
