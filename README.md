...
[![Version](https://img.shields.io/badge/version-v0.18.0-gold)](./CHANGELOG.md)
[![License](https://img.shields.io/badge/license-ISC-blue.svg)](./LICENSE)

**最新动态**：v0.18.0 后台资产深度审计 × 智能创作流程全面优化——发现并修复系统级矛盾（默认续写路径 TimeSliced 绕过约 90% 后台资产）。4 项断环修复（ingest→伏笔追踪、角色状态写入、内置 MCP 注册、四元组完整展开），4 项核心优化（TimeSliced 接入精选资产、3 个休眠技能激活、Inspector 全量上下文、审计触发改写建议），3 项死代码清理，3 项架构优化（Pro/Free 精细化分层、资产自动匹配、统一注入网关）。`cargo test --lib` 444/444 通过。

> **上一版**：v0.17.1 提示词注册表系统 + 两个紧急 Bug 修复 + 智能后台预访谈 + Anti-AI 改写闸 + 在世作者保护；v0.17.0 中文叙事增强四件套——31 张经典桥段卡、21 种剧情引擎、13 种高压关系、43 个体裁读者承诺；v0.16.2 修复后台审计 LLM 调用误导前端假超时；v0.16.0 智能创作参数全面可配置；v0.15.0 模型网关智能调度器；v0.14.3 场景智能路由（续写从必超时变为 30-60s 稳定生成）。
