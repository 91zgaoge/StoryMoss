...
每部小说将拥有独立的幕后工作室配置，存储在 `~/.config/storymoss/studios/{story_id}/` 目录下：
...
#### 导出功能
- 完整导出：导出所有配置到 `.storymoss` 文件（ZIP格式）
- 选择性导出：用户可选择要导出的配置模块

#### 导入功能
- 完整导入：从 `.storymoss` 文件导入所有配置
...
本次架构调整将使StoryMoss从一个传统的小说写作工具转变为一个真正智能的、以场景驱动创作的AI辅助写作平台。核心亮点包括：
