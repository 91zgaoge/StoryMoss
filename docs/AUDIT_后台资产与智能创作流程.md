...
StoryMoss 经过 v3 → v0.17 的持续建设，积累了**远超普通写作工具的后台资产**：52 种风格 DNA、5 种创作方法论、21 种剧情引擎、13 种高压关系、桥段卡库、体裁画像、知识图谱、规范状态、伏笔账本、记忆管线、自适应学习、技能系统、MCP 外部工具。这套资产的设计初衷是"越写越懂、AI 导演式创作"。
...
StoryMoss 的后台资产**设计完备、积累深厚**，在同类 AI 写作工具中属上乘。但审计揭示三个系统性问题：
...
> 确认日期：2026-06-20。基于 14 项修复全量实施后的代码库（v0.18.0），重新审计验证。

### 9.1 逐项验证结果

| 编号 | 修复项 | 状态 | 关键证据 |
|------|--------|------|----------|
| **P0-1** | ingest→伏笔闭环 | ✅ | `ingest.rs:272-274` 调用 `persist_foreshadowings`；`:970` INSERT 进 `foreshadowing_tracker` |
| **P0-2** | character_states 写入 | ✅ | `ingest.rs:279-281` 调用 `persist_character_states`；`:1052` INSERT OR REPLACE 进 `character_states` |
| **P0-3** | 内置 MCP 注册 | ✅ | `lib.rs:648-662` try_lock `BUILTIN_MCP_SERVER` → `registry.register` |
| **P0-4** | 四元组完整展开 | ✅ | `quartet_inference.rs:303-365` 序列化 works_with/pairs_well_with/category/when_to_use；`service.rs:2986-3052` 渲染 |
| **P1-1** | TimeSliced 精选资产 | ✅ | `write_time_bundle.rs:49-58` 含 5 个新字段；`to_prompt` 渲染全部 5 段；`orchestrator.rs:680-687` 从 task.parameters 注入四元组 |
| **P1-2** | 3 个休眠技能接通 | ✅ | `orchestrator.rs:1328/1359/1384` 调用 character_voice/plot_twist/text_formatter；辅助函数 `should_apply_*` 存在 |
| **P1-3** | Inspector 全量上下文 | ✅ | `service.rs:2325-2374` 注入世界观规则、待回收/逾期伏笔、叙事阶段、风格 DNA |
| **P1-4** | 审计触发改写建议 | ✅ | `events.rs:171-177` 含 `AuditRewriteSuggested` 变体；`audit_executor.rs:182-191` 为 high severity 发射事件 |
| **P2-1** | 死代码删除 | ✅ | `prompts/methodologies/`、`evolution/`、`state/`、`evolver.rs` 均不存在；`lib.rs` 无 `mod evolution`/`mod state` |
| **P2-2** | commentator registry | ✅ | `service.rs:1575` 使用 `resolve_prompt("commentator_system")`，不再硬编码 |
| **P2-3** | 死表删除 | ✅ | `connection.rs:3358-3364` Migration 94 DROP beat_cards/story_engines/pressure_relationships |
| **P3-1** | Pro/Free 分层 | ✅ | 单 StyleDNA（`:2010`）、写作风格详情（`:2028`）、作品简介（`:2062`）均移出 `is_pro`；StyleBlend（`:1980`）和方法论（`:1969`）保持 Pro-only |
| **P3-2** | 资产自动匹配 | ✅ | `orchestrator.rs:910-928` 按 `story.genre` 匹配 GenreProfile；显式设定优先 |
| **P3-3** | 统一注入网关 | ✅ | `asset_snapshot.rs` 存在；`creative_engine/mod.rs:7` 声明；`WriteTimeBundle` 和 `build_inspector_prompt` 均使用 |

### 9.2 修复后资产可达性矩阵

| 资产类别 | Full | TimeSliced（默认） | 变化 |
|----------|:---:|:---:|------|
| Skills（5 技能） | ✓ 全 5 技能链（P1-2） | ✗ 跳过 | P1-2：3 个从"永不触发"变为"场景智能触发" |
| 内置 MCP | ✗ | ✗ | P0-3：从"plan 步骤被丢弃"变为"可被 PlanGenerator 调度" |
| 方法论 | ✓ Pro | ✗ | 无变化 |
| StyleDNA（单一） | ✓ 全用户（P3-1） | ✔️ 经 style_dna_summary（P1-1） | P3-1：Free 可用；P1-1：TimeSliced 可用 |
| StyleBlend | ✓ Pro | ✗ | P3-1：保持 Pro-only |
| 写作风格详情 | ✓ 全用户（P3-1） | ✗ | P3-1：移出 is_pro |
| 作品简介 | ✓ 全用户（P3-1） | ✗ | P3-1：移出 is_pro |
| 个性化 | ✓ Pro | ✗ | 无变化 |
| 体裁画像 | ✓ | ✔️ 经 anti_patterns | 无变化 |
| 桥段卡/引擎/高压关系 | ✔️ 经四元组（P0-4 完整 payload） | ✔️ 经四元组（P1-1） | P0-4：payload 完整展开；P1-1：TimeSliced 接通 |
| 规范状态（阶段/冲突） | ✓ | ✔️ 经 narrative_phase_guidance（P1-1） | P1-1：TimeSliced 接通 |
| 伏笔/回收账本 | ✓ | ✔️ 经 pending/overdue（P1-1） | P1-1：TimeSliced 接通；P0-1：数据闭环 |
| 角色状态 | ✓（P0-2 写入） | ✗ | P0-2：表从"永远为空"变为"有数据" |

### 9.3 核心改善总结

1. **"资产黑洞"已解决**：TimeSliced（默认续写）现在接入叙事阶段、伏笔状态、主导风格摘要、叙事四元组四类精选资产，每项压缩为 1-3 行，总增量可控。用户最高频的"写第二章"操作能享受到后台资产红利。

2. **所有断环已接通**：伏笔自动追踪（P0-1）、角色状态写入（P0-2）、内置 MCP（P0-3）、四元组展开（P0-4）均实现闭环。写入的数据在下一次续写时被读取利用。

3. **死代码已清理**：删除 5 个死目录/文件、4 个 mod 声明、3 张死表，消除维护混淆。

4. **Free 用户资产增强**：单 StyleDNA、写作风格详情、作品简介从 Pro-only 变为全用户可用，Free 版也能享受基础风格约束。

5. **无回归**：`cargo test --lib` 444/444 通过，`cargo check` 零错误，`tsc --noEmit` 零错误。

6. **无新死代码**：验证确认所有新增代码（`asset_snapshot.rs`、辅助函数、事件变体）均被正确引用。

### 9.4 剩余可优化空间（非阻塞）

- `style_checker_system` 模板注册但未被实时路径调用（风格检查走规则法，LLM 风格检查未实现）
- `memory/query.rs` QueryPipeline 仍为死代码（保留为类型导出）
- 审计的 `AuditService`（5 维规则法）未被创作主流程调用（手动命令触发）

---

*本报告基于 2026-06-20 代码库快照，5 路并行 Agent 全量追踪 + v0.18.0 修复后逐项验证。所有 file:line 引用均经交叉验证。*
