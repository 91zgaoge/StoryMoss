...
description: StoryMoss 诊断与度量工具及解读指南（“用仪器量，别用眼睛看”）。何时加载：要定位卡在哪一步、要看生成链路日志、要截图、要跑集成 E2E、要验证 IPC/架构边界、要用 GitNexus 查调用图、或被问“怎么看当前状态/卡在哪/怎么截屏”时。
---
...
本项目已索引为 StoryMoss（14625 symbols / 24467 关系 / 293 执行流）。
...
- 重验证命令：
  ...
  - `ls scripts/`
  - `npx gitnexus status` 或读 `gitnexus://repo/StoryMoss/context`
