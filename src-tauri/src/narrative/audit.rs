...
//!
//! v5.3.0 新增功能：对已有故事项目进行结构分析，检测潜在问题。
//!
//! 检查维度：
...
impl StoryStructureAuditor {
    ...
    pub async fn analyze(&self, story_id: &str) -> Result<StoryAnalysisReport, String> {
        ...
        let overall_score = if total_weight > 0.0 {
            (dimensions.iter().map(|d| d.score as f32 * d.weight).sum::<f32>() / total_weight) as i32
        } else {
            0
        };
    ...
    async fn audit_foreshadowings(&self, story_id: &str) -> Result<AuditDimension, String> {
        ...
        let foreshadowings = tracker.get_all(story_id)
        ...
        let paid_off = foreshadowings.iter().filter(|f| matches!(f.status, TrackerStatus::Payoff)).count();
        let abandoned = foreshadowings.iter().filter(|f| matches!(f.status, TrackerStatus::Abandoned)).count();
        let pending = total - paid_off - abandoned;

        let score = if total > 0 {
            ((paid_off as f32 / total as f32) * 80.0 + (abandoned as f32 / total as f32) * 10.0) as i32
        } else { 0 };

        let mut details = vec![
            format!("总伏笔数: {}", total),
            format!("已回收: {} ({:.0}%)", paid_off, paid_off as f32 / total as f32 * 100.0),
            format!("待回收: {} ({:.0}%)", pending, pending as f32 / total as f32 * 100.0),
        ];
        if abandoned > 0 {
    ...
    async fn audit_characters(&self, story_id: &str) -> Result<AuditDimension, String> {
        ...
        let complete_chars = characters.iter().filter(|c| {
            c.personality.as_ref().map(|p| !p.is_empty()).unwrap_or(false)
                && c.goals.as_ref().map(|g| !g.is_empty()).unwrap_or(false)
                && c.background.as_ref().map(|b| !b.is_empty()).unwrap_or(false)
        }).count();
        ...
        Ok(AuditDimension {
            ...
            details: vec![
                format!("总角色数: {}", total),
                format!("信息完整: {} ({:.0}%)", complete_chars, complete_chars as f32 / total as f32 * 100.0),
            ],
        })
    }
    ...
    async fn audit_scenes(&self, story_id: &str) -> Result<AuditDimension, String> {
        ...
        let diversity_score = if unique_types >= 4 { 100 }
            else if unique_types >= 3 { 80 }
            else if unique_types >= 2 { 60 }
            else { 40 };
    ...
    async fn audit_world_building(&self, story_id: &str) -> Result<AuditDimension, String> {
        ...
        let wb = repo.get_by_story(story_id)
        ...
        if let Some(wb) = wb {
            ...
            let score = if rules_count >= 3 && has_history { 100 }
                else if rules_count >= 1 && has_history { 70 }
                else if rules_count >= 1 { 50 }
                else { 30 };
            ...
            Ok(AuditDimension {
                ...
                score,
                weight: 0.15,
                description: format!("{} 条世界规则{}", rules_count, if has_history { "，有历史背景" } else { "" }),
                details: vec![
    ...
    async fn audit_outline(&self, story_id: &str) -> Result<AuditDimension, String> {
        ...
        let outline = outline_repo.get_by_story(story_id)
            .map_err(|e| format!("读取大纲失败: {}", e))?;
        let scenes = scene_repo.get_by_story(story_id)
            .map_err(|e| format!("读取场景失败: {}", e))?;

        let actual_scenes = scenes.len() as i32;
        let estimated_scenes = outline.as_ref().and_then(|o| o.total_scenes_estimate).unwrap_or(0);

        let score = if estimated_scenes > 0 {
            let ratio = actual_scenes as f32 / estimated_scenes as f32;
            if ratio >= 0.8 && ratio <= 1.2 { 100 }
            else if ratio >= 0.5 { 70 }
            else { 40 }
        } else {
            if actual_scenes > 0 { 60 } else { 0 }
        };
        ...
        Ok(AuditDimension {
            ...
            score: score.max(0).min(100),
            weight: 0.20,
            description: format!("实际 {} 个场景 / 预估 {} 个", actual_scenes, estimated_scenes),
            details: vec![
    ...
    fn generate_recommendations(&self, dimensions: &[AuditDimension], findings: &[Finding]) -> Vec<String> {
        let mut recs = Vec::new();

        for dim in dimensions {
            if dim.score < 50 {
                recs.push(format!("【{}】{}（当前评分: {}）", dim.name, dim.description, dim.score));
            }
        }

        for finding in findings.iter().filter(|f| matches!(f.severity, FindingSeverity::Critical | FindingSeverity::Warning)) {
            recs.push(format!("【{}】{} — {}", finding.category, finding.message, finding.suggestion));
        }
