...
//!
//! 当预检发现缺少世界观合同或章节合同时，根据已有故事内容自动调用 LLM 生成并保存。
...
pub struct AutoContractProgress {
    pub stage: String, // "analyzing" | "generating_master" | "generating_chapter" | "saving" | "completed" | "error"
    pub message: String,
...
impl AutoContractBuilder {
    ...
    /// 自动补齐指定故事缺失的合同
    ///
    /// 返回 (是否创建了 master_setting, 是否创建了 chapter_contract)
    pub async fn auto_fill(
        ...
        if tree.master_setting.is_none() {
            ...
            match self.build_master_setting(story_id).await {
                Ok(contract) => {
                    self.emit_progress("saving", "世界观合同已生成并保存", 0.4);
                    created_master = true;
        ...
        if !has_chapter_contract {
            ...
            match self.build_chapter_contract(story_id, chapter_number).await {
                Ok(contract) => {
                    self.emit_progress("saving", "章节合同已生成并保存", 0.9);
        ...
        self.emit_progress("completed", "合同补齐完成", 1.0);
        Ok((created_master, created_chapter))
    }
