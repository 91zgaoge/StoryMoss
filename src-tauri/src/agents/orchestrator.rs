    ...
    async fn execute_trishot(
        ...
        if !is_genesis_first_chapter {
            ...
            if !continuation_ctx.is_empty() {
                final_prompt.push_str(&continuation_ctx);
            }
        }
        ...
        let call3_system_prompt_for_retry = system_prompt.clone();
        let (request_id, gen_response) = self
            .service
            .llm_service_ref()
        ...
        let mut request_id = call3_request_id;
        ...
        if !is_genesis_first_chapter {
            ...
            if crate::agents::trim_utils::should_retry_self_repetition(trim_ratio, raw_chars) {
                ...
                if remaining >= 30 {
                    let retry_timeout = remaining.min(call3_cap);
                    log::warn!(
                        "[TriShot-DIAG] 续写自重复检测 (ratio={:.2}, raw={} chars, cleaned={} chars), retrying with anti-repeat prompt (timeout={}s)",
                        trim_ratio, raw_chars, cleaned_chars, retry_timeout
                    );
                    self.workflow_log(
                        "trishot.call3.self_repetition_retry",
                        "续写检测到模型自重复，使用 anti-repeat 指令重试",
                        Some(serde_json::json!({
                            "task_id": task.id,
                    ...
                    let mut retry_prompt = call3_prompt_for_retry.clone();
                    retry_prompt.push_str(TRISHOT_ANTI_REPEAT_DIRECTIVE);
                    let (_retry_rid, retry_gen_response) = self
                        .service
                        .llm_service_ref()
                        .generate_for_task_with_tags_timeout_and_system_prompt(
                            crate::router::TaskType::CreativeWriting,
                            ...
                            Some("trishot-writer"),
                            call3_asset_tags_for_retry.clone(),
                            call3_selected_ids_for_retry.clone(),
                            Some(retry_timeout),
                            call3_system_prompt_for_retry.clone(),
                            trace_id.clone(),
                        )
                        .await;
                    let retry_rid = _retry_rid;

                    match retry_gen_response {
                        Ok(retry_resp) => {
                            let retry_raw = retry_resp.content;
                            let retry_content = sanitize_novel_output(&retry_raw);
                            if !retry_content.trim().is_empty() {
                                ...
                                log::info!(
                                    "[TriShot-DIAG] 续写自重复重试完成: retry_trim_ratio={:.2} (original={:.2}), accepted={}",
                                    retry_trim_ratio, trim_ratio, accepted
                                );
                                self.workflow_log(
                                    "trishot.call3.self_repetition_retry_done",
                                    "续写自重复重试完成",
                                    Some(serde_json::json!({
                                        "task_id": task.id,
                                        "original_trim_ratio": format!("{:.2}", trim_ratio),
                                        "retry_trim_ratio": format!("{:.2}", retry_trim_ratio),
                                        "accepted": accepted,
                                    })),
                                );
                                content = selected;
                        ...
                        Err(e) => {
                            log::warn!(
                                "[TriShot-DIAG] 续写自重复重试失败，保留首次清理结果: {}",
                                e
                            );
                        }
                    }
            ...
            if let Some(existing) = current_content_preview {
                ...
                let after_overlap = content.chars().count();
                if before_overlap > after_overlap {
                    log::info!(
                        "[TriShot-DIAG] 续写跨内容重叠剥离: {} → {} chars",
                        before_overlap,
                        after_overlap
                    );
                    self.workflow_log(
                        "trishot.call3.overlap_stripped",
                        "续写生成复述已有正文，已剥离重叠前缀",
                        Some(serde_json::json!({
                            "task_id": task.id,
                            "before_chars": before_overlap,
                            "after_chars": after_overlap,
                        })),
                    );
                }
            }
...
/// v0.26.24: 续写自重复重试时追加的反循环指令。
/// 当 TriShot 续写检测到模型自重复（意象/句式/场景块在单次生成内重复）时，
/// 用此指令强制重试，要求每段推进情节、禁止回环。
const TRISHOT_ANTI_REPEAT_DIRECTIVE: &str =
    "\n\n【绝对禁止 — 上一版违反了以下纪律，本次必须严格遵守】\n\
- 严禁任何句子、描写块、意象或场景在本次输出中出现两次或更多\n\
- 严禁回环到前文已写过的场景与意象，每一段必须是全新的情节推进\n\
- 严禁重复相同的句式结构（如「……的阴霾更加浓烈」「……的决心更加坚定」）\n\
- 严禁在结尾复述开头的场景、意象或句式\n\
- 请确保本次续写的每一段都向前推进情节，没有任何重复或循环";
...
fn build_continuation_context(
    ...
    // 2. 当前章尾部预览：Writer 需要看到正在续写的章节最后部分
...
pub(crate) fn sanitize_novel_output(content: &str) -> String {
    ...
    // 0. 剥离推理模型思考链、文件分隔符、编号规划块、markdown 代码块
    //    这些是推理模型在正文后泄漏的内部规划内容，必须先剥离再做后续清洗。
    let mut text = crate::narrative::strip_reasoning_blocks(content);

    // 0b. 剥离 +++++ 文件分隔符及其后所有内容（规划 markdown 泄漏入口）
    if let Some(pos) = text.find(|c: char| c == '+') {
    ...
    for (i, line) in lines_vec.iter().enumerate() {
        ...
        if is_numbered && plan_keywords.iter().any(|kw| t.contains(kw)) {
            ...
            consecutive += 1;
            if consecutive >= 3 {
                plan_block_start = first_numbered;
            }
        } else if t.is_empty() || line.starts_with(' ') || line.starts_with('\t') {
            // 空行或缩进行（子项）不重置计数器，它们是规划块的一部分
        } else {
            consecutive = 0;
            first_numbered = None;
        }
    ...
    if let Some(start) = plan_block_start {
        text = lines_vec[..start].join("\n");
        text = text.trim_end().to_string();
    }

    // 0d. 剥离 markdown 代码块（``` 包裹的行块）
...
mod tests {
    ...
    #[test]
    fn test_sanitize_strips_thinking_blocks() {
        // 推理模型思考链泄漏到正文前
        let raw = "<thinking>让我想想这个场景怎么写...</thinking>\n\n洞穴里很安静。";
        let cleaned = sanitize_novel_output(raw);
        assert!(!cleaned.contains("thinking"));
        assert!(!cleaned.contains("让我想想"));
        assert!(cleaned.contains("洞穴里很安静。"));
    }

    #[test]
    fn test_sanitize_strips_plus_delimiter_and_planning() {
        // +++++ 文件分隔符及规划 markdown 泄漏
        let raw = "洞穴里很安静。\n\n+++++ conversations/1_1_summon/0_character_concept.md\n1. 世界观设定\n2. 角色设定\n3. 故事结构";
        let cleaned = sanitize_novel_output(raw);
        assert!(cleaned.contains("洞穴里很安静。"));
        assert!(!cleaned.contains("+++++"));
        assert!(!cleaned.contains("conversations/"));
        assert!(!cleaned.contains("世界观设定"));
        assert!(!cleaned.contains("角色设定"));
    }

    #[test]
    fn test_sanitize_strips_numbered_planning_block() {
        // 编号规划块（无 +++++ 前缀，纯编号列表）
        let raw = "正文内容结束。\n\n1. 世界观设定\n   主题：末日\n2. 角色设定\n   主角：幸存者\n3. 故事结构\n   引言/开篇\n4. 叙事风格\n5. 具体情境构建\n6. 起草文本\n7. 最终润色";
        let cleaned = sanitize_novel_output(raw);
        assert!(cleaned.contains("正文内容结束。"));
        assert!(!cleaned.contains("世界观设定"));
        assert!(!cleaned.contains("叙事风格"));
        assert!(!cleaned.contains("最终润色"));
    }

    #[test]
    fn test_sanitize_strips_markdown_code_blocks() {
        // markdown 代码块泄漏
        let raw = "正文段落。\n\n```\n这是代码块内容\n不应该出现\n```\n\n另一段正文。";
        let cleaned = sanitize_novel_output(raw);
        assert!(cleaned.contains("正文段落。"));
