...
impl PlanTemplateLibrary {
    pub fn new(pool: DbPool) -> Self {
        let mut library = Self { templates: Vec::new(), pool };
    ...
    fn load_from_db(&mut self) -> Result<(), String> {
        ...
        let rows = stmt.query_map([], |row| {
            ...
            let plan: ExecutionPlan = serde_json::from_str(&plan_json).unwrap_or(ExecutionPlan {
        ...
        log::info!("[PlanTemplateLibrary] Loaded {} templates from DB", self.templates.len());
        Ok(())
    }
